...
def parse_option():
    ...
    parser.add_argument('--portion_dataset_coding', type=int, default=0,
                        help='[mnist, cifar10, imagenet_subset, gtsrb]')
    parser.add_argument('--portion_trigger_conding', type=int, default=0,
                        help='[patched_img, blending_img, filter_img]')
...
def set_default_settings(opt):
    ...
    if opt.inspect_layer_position is None:
        ...
        elif 'ResNet18' in opt.model:
            opt.inspect_layer_position = 2
        elif 'ResNetS' in opt.model:
            opt.inspect_layer_position = 2
        ...
        elif 'NeuralNet' in opt.model:
            opt.inspect_layer_position = 0
        elif 'dfb_gtsrb' in opt.model:
            opt.inspect_layer_position = 2
        else:
    ...
    if ('resnet' in opt.model and opt.inspect_layer_position >= 1) \
            or ('ResNet18' in opt.model and opt.inspect_layer_position >= 1) \
            or ('ResNetS' in opt.model and opt.inspect_layer_position >= 1) \
            ...
            or ('NeuralNet' in opt.model and opt.inspect_layer_position >= 1)\
            or ('dfb_gtsrb' in opt.model and opt.inspect_layer_position >= 1):
        opt.bound_on = True
    else:
        opt.bound_on = False
...
def load_model(opt):
    ...
    if 'resnet' in opt.model:
        ...
        model_classifier = partial_models_adaptive.ResNet18AdaptivePartialModel(
            num_classes=opt.n_cls,
            segment_layer_position=opt.inspect_layer_position,
            original_input_img_shape=(1, 3, opt.size, opt.size)
        )
    elif 'ResNetS' in opt.model:
        model_classifier = partial_models_adaptive.ResNetSAdaptivePartialModel(
            num_classes=opt.n_cls,
            segment_layer_position=opt.inspect_layer_position,
            original_input_img_shape=(1, 3, opt.size, opt.size)
        )
        ...
        if 'gtsrb' in opt.ckpt:
            model_classifier = partial_models_adaptive.NeuralNetAdaptivePartialModel(
                input_size=512,
    ...
    elif 'dfb' in opt.model:
        if 'gtsrb' in opt.model:
            model_classifier = partial_models_adaptive.GTSRBAdaptivePartialModel(
                num_classes=opt.n_cls,
                segment_layer_position=opt.inspect_layer_position,
                original_input_img_shape=(1, 3, opt.size, opt.size)
            )
    ...
    if 'bad_encoder_full_model' not in opt.model:
        ...
        if ('Troj' not in opt.ckpt) and ('flareon' not in opt.ckpt) and ('DRUPE' not in opt.ckpt) and ('Data-free_Backdoor' not in opt.ckpt) and ('Narcissus' not in opt.ckpt):
            try:
            ...
            if ('flareon' in opt.ckpt) or ('DRUPE' in opt.ckpt) or ('Data-free_Backdoor' in opt.ckpt) or ('Narcissus' in opt.ckpt):
                model_classifier.load_state_dict(ckpt)
            else:
    ...
    if torch.cuda.is_available():
        ...
        if ('Troj' not in opt.ckpt) and ('flareon' not in opt.ckpt) and ('DRUPE' not in opt.ckpt) and ('Data-free_Backdoor' not in opt.ckpt) and ('Narcissus' not in opt.ckpt) and ('bad_encoder_full_model' not in opt.model):
