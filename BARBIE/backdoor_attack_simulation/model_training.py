...
def parse_option():
    ...
    parser.add_argument('--data_folder', type=str,
                        default=
                        '/BARBIE/datasets/CIFAR10',
                        # '/BARBIE/datasets/GTSRB',
                        # '/BARBIE/datasets/ImageNetSubset',
                        # '/BARBIE/datasets/',
                        # '/BARBIE/datasets/ImageNet',
    ...
    parser.add_argument('--blending_trigger_path', type=str,
                        default='/BARBIE/backdoor_attack_simulation/triggers/',
                        help='folder of the image that is set as the blending trigger')

    # special
    parser.add_argument('--my_marker', type=str, default='',
...
def set_model(opt):
    if opt.model == 'resnet50':
        ...
        model.load_state_dict(torch.load('/BARBIE/backdoor_attack_simulation/resnet50-19c8e357.pth'))
    ...
    if os.path.exists(save_file):
        ...
        with tqdm(total=opt.epochs) as t:
            for epoch in range(1, opt.epochs + 1):
                ...
                print('Epoch: {}, Test_Acc: {:.2f}'.format(epoch, test_acc))
                ...
                if 'poison' in opt.dataset:
                    ...
                    if test_acc.item() > 95.:
                        print('Standard satisfied. Stop training and saving...')
                        break
                    elif 'stl10' in opt.dataset and test_acc.item() > 93.:
                        print('Standard satisfied. Stop training and saving...')
                        break
        ...
        # save the last model
        if opt.my_marker == '':
            save_file = os.path.join(opt.save_folder, f'last.pth')
        else:
            save_file = os.path.join(opt.save_folder, f'last_{opt.my_marker}.pth')
