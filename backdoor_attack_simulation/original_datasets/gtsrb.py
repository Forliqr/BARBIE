...
class GTSRB(VisionDataset):
    ...
    def __init__(self, root, train=True, transform=None, download=False):
        ...
        if not self.train:
            df_info_path = os.path.join(root, 'GT-final_test.csv')
            root = os.path.join(root, 'Final_Test/Images')
            self.root = root
        else:
            df_info_path = os.path.join(root, 'GT-training.csv')
            root = os.path.join(root, 'Training')
            self.root = root

        self.df_info = pd.read_csv(df_info_path, sep=';')
        ...
        for sample_id in tqdm(range(length_df_info)):
            _path = os.path.join(self.root, self.df_info['Filename'].loc[sample_id])
