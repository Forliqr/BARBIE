...
class ImageNet(VisionDataset):
    ...
    def __init__(self, root, train=True, transform=None, download=False):
        ...
        label_names = [label_name for label_name in label_names if not label_name.startswith('.')]
        self.n_cls = len(label_names)
        # print(label_names)
        # print('There are '+str(self.n_cls)+' classes!')
        self.img_size = 224
        ...
        with tqdm(total=len(label_names)) as _tqdm:
            for label_int in range(len(label_names)):
                ...
                for file_name in os.listdir(label_path):
                    ...
                    img_shape = img.shape
                    if img_shape != (self.img_size, self.img_size, 3):
                        continue
