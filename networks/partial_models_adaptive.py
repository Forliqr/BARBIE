...
class NeuralNetAdaptivePartialModel(NeuralNet):
    def __init__(self, input_size, hidden_size_list, num_classes):
        super(NeuralNetAdaptivePartialModel, self).__init__(input_size, hidden_size_list, num_classes)
        # segment_layer_position indicates which layer to inspect on.
        self.input_shapes = []
        template_original_input = torch.ones(size=(1, input_size))
...
class ResNetAdaptivePartialModel(ResNet):
    ...
    def adaptive_forward(self, x):
        if self.segment_layer_position in [0]:
            x = F.relu(self.bn1(self.conv1(x)))
        # shape=[1,64,32,32]
        if self.segment_layer_position in [0, 1]:
            x = self.layer1(x)
        # shape=[1,64,32,32]
        if self.segment_layer_position in [0, 1, 2]:
            x = self.layer2(x)
        # shape=[1,128,8,8]
        if self.segment_layer_position in [0, 1, 2, 3]:
            x = self.layer3(x)
        # shape=[1,256,32,32]
