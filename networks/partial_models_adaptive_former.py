...
class ResNetAdaptivePartialModel(ResNet):
    ...
    def __init__(self, layer_setting, block_setting, num_classes=10, segment_layer_position=-1,
        ...
        super(ResNetAdaptivePartialModel, self).__init__(num_classes=num_classes,
                                                         ...
                                                         block=block_setting)
        # segment_layer_position indicates which layer to inspect on.
        self.inspect_layer_positions = [0, 1, 2, 3, 4, 5]
        self.segment_layer_position = self.inspect_layer_positions[segment_layer_position]
        self.input_shapes = []
    ...
    def adaptive_forward(self, x):
        if self.segment_layer_position in [1, 2, 3, 4, 5]:
            x = F.relu(self.bn1(self.conv1(x)))
        # shape=[1,64,32,32]
        if self.segment_layer_position in [2, 3, 4, 5]:
            x = self.layer1(x)
        # shape=[1,64,32,32]
        if self.segment_layer_position in [3, 4, 5]:
            x = self.layer2(x)
        # shape=[1,128,8,8]
        if self.segment_layer_position in [4, 5]:
            x = self.layer3(x)
        # shape=[1,256,32,32]
        if self.segment_layer_position in [5]:
            x = self.layer4(x)
            x = self.avgpool(x)
        # # shape=[1,512,1,1]
        # if self.segment_layer_position in [0, 1, 2, 3, 4, 5]:
        #     x = torch.flatten(x, 1)
        #     x = self.fc(x)
        return x
...
class VGGAdaptivePartialModel(VGG16):
    ...
    def __init__(self, num_classes=10, in_dims=512, segment_layer_position=1,
        ...
        self.segment_layer_position = self.inspect_layer_positions[segment_layer_position]
        print(f'self.segment_layer_position:{self.segment_layer_position}')
        self.input_shapes = []
        template_original_input = torch.ones(size=original_input_img_shape)
        self._forward_record_input_shapes(template_original_input)
    ...
    def adaptive_forward(self, x):
        ...
        # print(x.shape)
        if self.segment_layer_position in [1, 2, 3, 4]:
            for layer in range(3, 14):
                x = self.features[layer](x)
        # print(x.shape)
        if self.segment_layer_position in [2, 3, 4]:
            for layer in range(14, 24):
                x = self.features[layer](x)
        # print(x.shape)
        if self.segment_layer_position in [3, 4]:
            for layer in range(24, 34):
                x = self.features[layer](x)
        # print(x.shape)
        if self.segment_layer_position in [4]:
            for layer in range(34, 44):
                x = self.features[layer](x)
            x = x.view(x.size(0), -1)  # shape=[1,512]
        # print(x.shape)

        # if self.segment_layer_position in [0, 1, 2, 3, 4]:
...
class SimpleCNNAdaptivePartialModel(SimpleCNN):
    ...
    def adaptive_forward(self, x):
        if self.segment_layer_position in [1]:
            if len(x.size()) == 3:
                x = x.unsqueeze(0)
            n = x.size(0)
            x = self.m1(x)
            ...
            x = x.view(n, -1)
        # if self.segment_layer_position in [0, 1]:
        #     x = self.m2(x)
        return x
...
class GoogLeNetAdaptivePartialModel(GoogLeNet):
    ...
    def __init__(self, num_classes=10, segment_layer_position=2,
